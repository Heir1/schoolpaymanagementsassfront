...
export default function EditStudentFeePage() {
  ...
  const fetchData = useCallback(() => {
    ...
    setLoading(true);
    Promise.all([
      api.admin.getStudentFee(token, studentId, feeId),
  ...
  useEffect(() => {
    ...
    if (!studentId || !feeId) return;
    fetchData();
  }, [user?.role.name, router, studentId, feeId, fetchData]);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    api.admin
      .updateStudentFee(token, studentId, feeId, body)
      .then(() => router.push(`/dashboard/students/${studentId}/fees/${feeId}`))
      .catch((e) => {
  ...
  return (
    <div className="p-6 lg:p-8">
      <Link
        href={`/dashboard/students/${studentId}/fees/${feeId}`}
