...
export default function FeesPage() {
  ...
  const [list, setList] = useState<AdminFeeListItem[]>([]);
  const [pagination, setPagination] = useState<FeeTypePagination | null>(null);
  const [stats, setStats] = useState<AdminFeesStatisticsResponse["data"] | null>(null);
  ...
  useEffect(() => {
    ...
    fetchFees();
    fetchStats();
  }, [user?.role.name, router, fetchFees, fetchStats]);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-600">
            Gestion des frais (montant, échéance, tranches, classes associées).
          </p>
        </div>
        <Link
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <div className="rounded-xl bg-white border border-slate-200/80 shadow-card p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-slate-900">{stats.general.total_fees}</p>
                <p className="text-xs text-slate-500">Total frais</p>
              </div>
            </div>
          </div>
          <div className="rounded-xl bg-white border border-slate-200/80 shadow-card p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-slate-900">
                  {formatAmount(stats.general.total_amount)}
                </p>
          ...
          <div className="rounded-xl bg-white border border-slate-200/80 shadow-card p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-slate-900">
                  {stats.general.fees_with_installments}
                </p>
                <p className="text-xs text-slate-500">Avec tranches</p>
              </div>
            </div>
          </div>
          <div className="rounded-xl bg-white border border-slate-200/80 shadow-card p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-slate-900">
                  {formatAmount(stats.general.average_amount)}
                </p>
                    ...
                    filteredList.map((item) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-sm text-center text-slate-600">
                            {item.associated_classes_count ?? (item.associated_classes?.length ?? 0)}
                          </td>
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2 flex-wrap">
                              <Link
                                href={`/dashboard/fees/${item.id}`}
                              ...
                              {!isDeleted ? (
                                <>
                                  <Link
                                    href={`/dashboard/fees/${item.id}/edit`}
