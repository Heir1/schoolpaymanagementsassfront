...
export default function NewStudentGroupFeePage() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    api.admin
      .getFeeTypes(token, 1)
      .then((res) => {
        const types = (res.data.fee_types ?? []).filter((t) => !t.deleted_at);
        setFeeTypes(types);
        if (types.length > 0 && !feeTypeId) setFeeTypeId(String(types[0].id));
      })
      .catch(() => setError("Impossible de charger les types de frais."))
      .finally(() => setLoading(false));
  }, [user?.role.name, router, groupId]);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    api.admin
      .createStudentGroupFee(token, groupId, body)
  ...
  return (
    <div className="p-6 lg:p-8">
      ...
      <h1 className="font-display text-2xl font-bold text-slate-900 tracking-tight mb-2">
        Ajouter un frais (groupe d&apos;élèves)
      </h1>
      <p className="text-slate-600 mb-8">
        Type, montant, échéance. Optionnel : tranches (somme = montant). Pas de classes associées — le groupe est déjà défini.
      </p>
