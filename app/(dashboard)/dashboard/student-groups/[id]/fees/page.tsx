...
export default function StudentGroupFeesPage() {
  const { user } = useAuth();
  const router = useRouter();
  const params = useParams();
  const groupId = params?.id as string | undefined;

  const [groupRef, setGroupRef] = useState<{ name: string } | null>(null);
  const [list, setList] = useState<AdminStudentGroupFeeListItem[]>([]);
  ...
  const fetchFees = useCallback(() => {
    ...
    setError(null);
    api.admin
      .getStudentGroupFees(token, groupId, page)
      .then((res) => {
        setGroupRef(res.data.student_group);
  ...
  useEffect(() => {
    ...
    if (!groupId) return;
    fetchFees();
  }, [user?.role.name, router, groupId, fetchFees]);
  ...
  return (
    <div className="p-6 lg:p-8">
      <Link
        href={`/dashboard/student-groups/${groupId}`}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-600">
            {groupRef ? groupRef.name : "Chargement…"}
          </p>
        </div>
        <Link
          href={`/dashboard/student-groups/${groupId}/fees/new`}
                  ...
                  {filteredList.length === 0 ? (
                    <tr>
                      <td colSpan={4} className="px-4 py-12 text-center text-slate-500 text-sm">
                        {filter === "active"
                          ? "Aucun frais actif pour ce groupe."
                          : filter === "trash"
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2 flex-wrap">
                              <Link
                                href={`/dashboard/student-groups/${groupId}/fees/${item.id}`}
                              ...
                              {!isDeleted && (
                                <>
                                  <Link
                                    href={`/dashboard/student-groups/${groupId}/fees/${item.id}/edit`}
