...
import type { AdminStudentGroupFeeDetailResponse } from "@/lib/types";
...
export default function StudentGroupFeeDetailPage() {
  const { user } = useAuth();
  ...
  const [data, setData] = useState<AdminStudentGroupFeeDetailResponse["data"] | null>(null);
  ...
  const fetchFee = useCallback(() => {
    ...
    setError(null);
    api.admin
      .getStudentGroupFee(token, groupId, feeId)
  ...
  useEffect(() => {
    ...
    if (!groupId || !feeId) return;
    fetchFee();
  }, [user?.role.name, router, groupId, feeId, fetchFee]);

  function handleDelete() {
    if (!confirm("Supprimer ce frais ?")) return;
    const token = getToken();
    if (!token || !groupId || !feeId) return;
    setActionLoading("delete");
    api.admin
      .deleteStudentGroupFee(token, groupId, feeId)
      .then(() => router.push(`/dashboard/student-groups/${groupId}/fees`))
      .catch((e) => {
  ...
  function handleRestore() {
    ...
    api.admin
      .restoreStudentGroupFee(token, groupId, feeId)
      .then(() => fetchFee())
  ...
  if (error && !data) {
    return (
      <div className="p-6 lg:p-8">
        <Link
          href={`/dashboard/student-groups/${groupId}/fees`}
  ...
  const fee = data as AdminStudentGroupFeeDetailResponse["data"];
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          {!isDeleted && (
            <>
              <Link
                href={`/dashboard/student-groups/${groupId}/fees/${feeId}/edit`}
