...
export default function EditStudentGroupPage() {
  ...
  useEffect(() => {
    ...
    api.admin
      ...
      .catch((e) => setError(e instanceof Error ? e.message : "Groupe non trouvé."))
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    api.admin
      .updateStudentGroup(token, id, {
        name: name.trim(),
        description: description.trim() || undefined,
      })
      .then(() => router.push(`/dashboard/student-groups/${id}`))
      .catch((e) => {
  ...
  return (
    <div className="p-6 lg:p-8">
      <Link
        href={`/dashboard/student-groups/${id}`}
        className="inline-flex items-center gap-2 text-sm font-medium text-slate-600 hover:text-slate-900 mb-6"
      >
        <ArrowLeft className="w-4 h-4" />
        Retour au détail
